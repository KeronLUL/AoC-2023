...
void check_symbol(std::vector<std::tuple<int, char, int>> symbols, std::tuple<int, int, int> tuple,
                  std::map<std::pair<size_t, size_t>, std::tuple<size_t, size_t>> *map, size_t *sum_part1){
...
void solution(std::istream &input, size_t *sum_part1, size_t *sum_part2){
    std::string line;
    std::vector<std::vector<std::tuple<int, char, int>>> symbols;
    std::vector<std::vector<std::tuple<int, int, int>>> numbers;
    size_t line_count = 0;

    while (std::getline(input, line)){
        std::string symbols_line = line;
        std::string numbers_line = line;
        std::vector<std::tuple<int, char, int>> line_symbols;
        std::vector<std::tuple<int, int, int>> line_numbers;
        std::smatch match;
        size_t pos_sym = 0;
        size_t pos_num = 0;
        line_count++;
    ...
    std::map<std::pair<size_t, size_t>, std::tuple<size_t, size_t>> map;
